...
エージェント間の意思決定は、影響度に応じたレベル別権限と合意形成プロセスで管理されます。

#### **決定権限レベル概要**

- **Level 0**: 人間の最終判断（プロジェクト方向性・ビジネス要件）
- **Level 1**: 各エージェントの自律判断（専門領域内の技術的決定）
- **Level 2**: 関連エージェント協議（複数エージェントに影響する変更）
- **Level 3**: 全体合意 + 人間承認（プロジェクト全体に影響する根本変更）

#### **合意形成の基本フロー**

1. **提案・通知**: GitHub PR/Issue経由での変更提案
2. **自動通知**: CODEOWNERS・Discord連携による関係者通知  
3. **協議・レビュー**: 影響度に応じた関係者での討議
4. **決定・承認**: レベル別承認プロセス
5. **実装・共有**: 決定事項の実装と全体共有

**詳細**: `/docs/agents/decision_making_process.md` - 詳細な意思決定プロセスと実装方法

#### **レビュー・通知システム**

GitHub PR/Issue作成時の自動レビュアーアサインと Discord通知により、効率的なレビュープロセスを実現しています。

**レビュー通知の基本フロー**:

1. **PR/Issue作成** → **CODEOWNERS自動アサイン** → **Discord通知**
2. **レビュー実行** → **承認・マージ** → **完了通知**

**期限管理**: 24時間以内レビュー目標、期限切れ自動警告
**優先度別通知**: Level別の通知チャンネル分散

**詳細**: `/docs/agents/decision_making_process.md` - レビュー通知システムの実装詳細
