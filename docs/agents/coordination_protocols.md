...
Task(
  ...
  description="API設計相談",
  prompt="バックエンドエージェントへ相談: シーンサービスとムービーサービス間のAPI連携について、パフォーマンスと設計の観点から最適なアプローチを教えてください。現在の検討案: [具体的な案を記載]"
)
...
Task(
  ...
  prompt="データベースエージェントとアーキテクトエージェントへレビュー依頼: マイクロサービス間のデータ整合性確保について、添付のER図とAPI仕様をレビューし、改善提案をお願いします。"
)
```
